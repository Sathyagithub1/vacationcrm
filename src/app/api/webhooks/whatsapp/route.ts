...
export async function GET(request: NextRequest) {
  ...
  // verifySignature on GET: pass the verify_token as body, empty headers
  // WhatsApp adapter checks x-hub-signature-256 normally, but for verification
  // we check the token against the stored verifyToken in credentials.
  const credentials = JSON.parse(
    require("@/lib/encryption").decrypt(config.credentials)
  ...
  void adapter; // adapter loaded for consistency
